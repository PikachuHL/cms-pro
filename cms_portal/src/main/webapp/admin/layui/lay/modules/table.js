    ...
    Class.prototype.renderData = function(res, curr, count, sort){
        var that = this
            ...
            ,render = function(){ //后续性能提升的重点
                ...
                layui.each(data, function(i1, item1){
                    ...
                    that.eachCols(function(i3, item3){
                        ...
                        if(fieldArray.length>0){
                            (item1[fieldArray[0]]) && (content=(item1[fieldArray[0]][fieldArray[1]])?(item1[fieldArray[0]][fieldArray[1]]):content)
                        }
                    ...
                    //-------------------------------我们自己添加的form----------------------------------
                    if(options.wrapperForm){
                        let form=$('<form class="layui-form" method="post">').append('<tr data-index="'+ i1 +'">'+ tds.join('') + '</tr>');
                        trs.push(form);
                    }else{
                ...
                //-----------------------------------我们自己添加的form--------------------------------------
                // wrapperForm属性存在的话,会给每个tr包裹一个form表单,进行单独的验证
                if(options.wrapperForm){
                    that.layMain.find('tbody').empty();
                    trs.forEach(function(item){
                        that.layMain.find('tbody').append(item)
                    });
                }else{
